        ...
        <section className="grid flex-1 gap-6 xl:grid-cols-[minmax(0,2fr)_420px]">
          <div className="rounded-[2rem] border border-white/70 bg-white/80 p-5 shadow-[0_24px_70px_rgba(15,23,42,0.08)] backdrop-blur">
            <div className="mb-5 flex items-center justify-between gap-4">
              ...
              <div className="rounded-full border border-cyan-200 bg-cyan-50 px-4 py-2 text-sm font-medium text-cyan-800">
                Dwell timer: 3.0s
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {ACTIONS.map((action) => (
                <PatientButton
                  ...
                  progress={focusedActionId === action.id ? dwellProgress : 0}
                  onActivate={() => void handleActionTrigger(action.id)}
                  refCallback={registerButton(action.id)}
                />
              ))}
            </div>
            ...
            <div className="mt-5">
              <PatientButton
                ...
                isFocused={focusedActionId === EMERGENCY_ACTION.id}
                progress={focusedActionId === EMERGENCY_ACTION.id ? dwellProgress : 0}
                onActivate={() => void handleActionTrigger(EMERGENCY_ACTION.id)}
                refCallback={registerButton(EMERGENCY_ACTION.id)}
              />
            </div>
          </div>

          <CaretakerPanel events={events} />
        </section>
      </main>
    </div>
  );
